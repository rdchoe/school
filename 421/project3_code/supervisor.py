...
import proj3					# File containing your programs for Project 3


def main(problem=rhook16b, time_limit=300, draw=1):
	"""
	...
	if 'initialize' in dir(proj3):
		print('Calling proj2.initialize.')
		p = mp.Process(target=proj3.initialize, \
			args = (((x,y),(u,v)), f_line, walls, ))
		p.start()
		# Wait for 10 seconds (the time limit I specified on Piazza)
		p.join(300)
		if p.is_alive():
			print('\nWarning: terminating proj2.initialize at 15 seconds.')
	...
	while True:
		...
		if goal_test((x,y), (u,v), f_line):
			print('\nYour program completed a successful run with path length: {}'.format(path_length))
			break
...
def get_proj2_choice(position, velocity, f_line, walls, time_limit):
	...
	p = mp.Process(target=proj3.main, \
