"""
File: make_random_probs.py -- Dana Nau <nau@cs.umd.edu>, Sept 13, 2018

Code to make random racetrack problems. The main program creates problems and
prints them to standard output, so if you want to put them in a file you should
either cut-and-paste from the command-line window or redirect standard output.
...
def make_one(size=48, display=0, title='', density=1, complexity=1):
    ...
    maze of dimensions size/2 x size/2. Next, add a starting point and finish line.
    Then, so the distance between adjacent walls won't be too small, double all of
    the coordinates, to get a racetrack of dimensions size x size.
    """
