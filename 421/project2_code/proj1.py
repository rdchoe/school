...
def corn(walls,fline):
    ...
    for corner in s_corners:
        dist = edistw_to_finish(corner, fline, walls)
        if dist != infinity:
    ...
    if s_corners:
        while i < max_limit:
            ...
            for corner in s_corners:
                temp = None
                dist = edistw_to_finish(corner, [stack[0][0], stack[0][0]], walls)
...
def h_proj1(state, fline, walls):
    ...
    '''
    If the state has not straight line distance to the finish line wihtout any walls between,
    find the distance to the nearest corner point + that corner's point to the finish line
'''    '''
    else:
        mininum = math.inf
        for i, c in enumerate(stack):
            distance_to_corner = edistw_to_finish((x, y), [(c[0][0],c[0][1]),(c[0][0],c[0][1])], walls)
            corner_to_fline = c[1]
