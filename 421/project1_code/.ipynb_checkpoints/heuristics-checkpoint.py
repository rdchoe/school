"""
File: heuristics.py -- Dana Nau, Sept 10, 2018
...
- h_walldist computes the approximate distance to the goal without ignoring walls. 
 ...
 Each heuristic function takes three arguments: state, fline, walls.
   ...
   - fline is the finish line. It should have the form ((x1,y1), (x2,y2)), 
      and should be either vertical (x1 == x2) or horizontal (y1 == y2)
...
def corn(walls):
    global corners, s_corners, xmax, ymax
    ...
    for wall in walls:
        if wall[1] in start:
            corners.append(wall[1])
    for c in corners:
    
        if not in_wall((c[0]+1, c[1]+1), walls):
            s_corners.append((c[0]+1, c[1]+1)) #right
        if not in_wall((c[0]-1, c[1]-1), walls):
            s_corners.append((c[0]-1, c[1]-1)) #left
        if not in_wall((c[0]+1, c[1]-1), walls):
            s_corners.append((c[0]+1, c[1]-1)) #up
        if not in_wall((c[0]-1, c[1]+1), walls):
            s_corners.append((c[0]-1, c[1]+1)) #down     
        
        
        
    
def h_proj1(state, fline, walls):
    global s_corners, xmax, ymax
    if corners == []:
...
def h_walldist(state, fline, walls):
    ...
    return hval
'''
def h_corner(state, fline, walls):
    #compute all corners and store
    global corners 
    if corners == []:
        get_corners(walls)


def get_corners(walls):
...
def edist_grid(fline,walls):
    ...
    xmax = max([max(x,x1) for ((x,y),(x1,y1)) in walls])
    ymax = max([max(y,y1) for ((x,y),(x1,y1)) in walls])
    grid = [[edistw_to_finish((x,y), fline, walls) for y in range(ymax+1)] for x in range(xmax+1)] #my heuristic will choose specific points instead of all
    flag = True
...
#       return 0
    ...
    ds.append(math.inf)    # for the case where ds is empty
    return min(ds)
